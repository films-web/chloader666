...
class AppController {
public:
    static __forceinline void Register(EventBus& bus, SessionContext& ctx, MessageBroker& broker, IPCServer& ipc, const std::string& root, const std::string& exe) {
        ...
        bus.Subscribe(EventType::INJECT_PAYLOAD, [&bus, &ctx, exe, root](const Event&) {
            std::thread([&bus, &ctx, exe, root]() {
                ...
                if (Injector::GetProcessIdByName(Constants::TargetExe().c_str()) != 0) {
                    bus.Publish({ EventType::UI_STATUS_UPDATE, std::make_pair(UiStatusType::ERROR_STATE, std::string(PCrypt("Error: Game already running!").c_str())) });
                    std::this_thread::sleep_for(std::chrono::seconds(3));
                    bus.Publish({ EventType::SHUTDOWN_REQUESTED, std::monostate{} });
                    return;
                }
                ...
                PayloadHandler::Session ghostSession;

                bus.Publish({ EventType::UI_STATUS_UPDATE, std::make_pair(UiStatusType::LOADING, std::string(PCrypt("Syncing Anti-Cheat...").c_str())) });

                if (!PayloadHandler::CommitToDisk(targetPathStr, ctx.GetDllBytes(), ghostSession)) {
                    bus.Publish({ EventType::UI_STATUS_UPDATE, std::make_pair(UiStatusType::ERROR_STATE, std::string(PCrypt("FS Error: Access Denied").c_str())) });
                    std::this_thread::sleep_for(std::chrono::seconds(3));
                    bus.Publish({ EventType::SHUTDOWN_REQUESTED, std::monostate{} });
                    return;
                }
                ...
                std::string actualHash = Crypto::CalculateSHA256File(targetPathStr.c_str());

                if (expectedHash.length() == actualHash.length() &&
                    std::equal(expectedHash.begin(), expectedHash.end(), actualHash.begin(),
                        [](char a, char b) { return std::tolower(a) == std::tolower(b); })) {

                    bus.Publish({ EventType::UI_STATUS_UPDATE, std::make_pair(UiStatusType::LOADING, std::string(PCrypt("Injecting...").c_str())) });

                    DWORD pid = Injector::LaunchAndInject(exe, targetPathStr.c_str(), [](HANDLE hProc, DWORD pid, DWORD_PTR baseAddr) {
                    ...
                    bus.Publish({ EventType::UI_STATUS_UPDATE, std::make_pair(UiStatusType::ERROR_STATE, std::string(PCrypt("Error: Hash Mismatch!").c_str())) });
                    std::this_thread::sleep_for(std::chrono::seconds(3));
                    bus.Publish({ EventType::SHUTDOWN_REQUESTED, std::monostate{} });
                }
                }).detach();
